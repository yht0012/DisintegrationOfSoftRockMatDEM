...
interface=[0.0667;0.734;0.933];%interface between layers
maxZ=max(d.mo.aZ);
botLayerFilter=d.mo.aZ<maxZ*interface(1);
midLayer1Filter=d.mo.aZ>=maxZ*interface(1)&d.mo.aZ<=maxZ*interface(2);
midLayer2Filter=d.mo.aZ>=maxZ*interface(2)&d.mo.aZ<=maxZ*interface(3);
topLayerFilter=d.mo.aZ>maxZ*interface(3);
d.addGroup('botLayer',find(botLayerFilter));
d.addGroup('midLayer',find(midLayer1Filter));
d.addGroup('midLayer',find(midLayer2Filter));
d.addGroup('topLayer',find(topLayerFilter));
%f.run('fun/setGroupId.m',d);d.show('groupId');
